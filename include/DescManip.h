...
public:
   ...
   */
  static void meanValue(const std::vector<TDescriptor> &descriptors,
                        TDescriptor &mean);
  ...
  static void toStream(const TDescriptor &m, std::ostream &str);
  static void fromStream(cv::Mat &m, std::istream &str);
  static void fromStream(TDescriptor &m, std::istream &str);
...
uint32_t DescManip::distance_8uc1(const TDescriptor &a, const TDescriptor &b)
{
  //binary descriptor
  ...
  uint64_t v, ret = 0;
  int n = sizeof(TDescriptor) / sizeof(uint64_t);
