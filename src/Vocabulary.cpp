...
// {
//   std::vector<std::vector<TDescriptor>> vtf(training_features.size());
    ...
    const std::vector<std::vector<TDescriptor>> &training_features)
{
  ...
  std::vector<TDescriptor> features;
...
void Vocabulary::getFeatures(
    const std::vector<std::vector<TDescriptor>> &training_features,
    std::vector<TDescriptor> &features) const
{
  features.resize(0);
  for (size_t i = 0; i < training_features.size(); i++)
...
void Vocabulary::HKmeansStep(NodeId parent_id,
                             const std::vector<TDescriptor> &descriptors, int current_level)
{
  ...
  else
  {
    ...
    while (goon)
    {
      ...
      else
      {
        ...
        {
          std::vector<TDescriptor> cluster_descriptors;
  ...
  if (current_level < m_L)
  {
    ...
    for (unsigned int i = 0; i < clusters.size(); ++i)
    {
      ...
      std::vector<TDescriptor> child_features;
...
void Vocabulary::initiateClusters(const std::vector<TDescriptor> &descriptors,
...
void Vocabulary::initiateClustersKMpp(
    const std::vector<TDescriptor> &pfeatures,
    std::vector<TDescriptor> &clusters) const
{
...
void Vocabulary::setNodeWeights(const std::vector<std::vector<TDescriptor>> &training_features)
{
...
TDescriptor Vocabulary::getWord(WordId wid) const
{
...
// WordId Vocabulary::transform(const TDescriptor &feature) const
// {
//   if (empty())
//   {
//     return 0;
...
//     const TDescriptor &features, BowVector &v) const
// {
//   //    std::vector<TDescriptor> vf(features.rows);
...
void Vocabulary::transform(const TDescriptor &feature, WordId &id) const
{
  WordValue weight;
...
void Vocabulary::transform(const TDescriptor &feature,
